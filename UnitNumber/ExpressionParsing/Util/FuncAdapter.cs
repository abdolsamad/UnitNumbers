...
namespace UnitConversionNS.ExpressionParsing.Util
{
    ...
    internal class FuncAdapter
    {
        ...
        public Delegate Wrap(IEnumerable<Execution.ParameterInfo> parameters, 
            Func<IDictionary<string, ExecutionResult>, ExecutionResult> function)
        {
        ...
        private Delegate GenerateDelegate(Execution.ParameterInfo[] parameterArray,
            Func<Dictionary<string, ExecutionResult>, ExecutionResult> function)
        {
            Type delegateType = GetDelegateType(parameterArray);
            Type dictionaryType = typeof(Dictionary<string, ExecutionResult>);

            LabelTarget returnLabel = Expression.Label(typeof(ExecutionResult));

            ParameterExpression dictionaryExpression =
                Expression.Variable(typeof(Dictionary<string, ExecutionResult>), "dictionary");
            ...
            for (int i = 0; i < parameterArray.Length; i++)
            {
                ...
                Type parameterType = parameterArray[i].DataType == DataType.UnitNumber ? typeof(UnitNumber) : typeof(double);
                ...
                methodBody.Add(Expression.Call(dictionaryExpression,
                    dictionaryType.GetMethod("Add", new Type[] { typeof(string), typeof(ExecutionResult) }),
                    Expression.Constant(parameterArray[i].Name),
                    Expression.Convert(parameterExpressions[i], typeof(ExecutionResult)))
                    );
            }
        ...
        private Type GetDelegateType(Execution.ParameterInfo[] parameters)
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
                typeArguments[i] = (parameters[i].DataType == DataType.UnitNumber) ? typeof(UnitNumber) : typeof(double);
